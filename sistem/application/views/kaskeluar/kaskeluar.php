...
<div class="row">
	...
	<form class="new-added-form" action="<?php echo base_url(); ?>kaskeluar/savekeluar" method="post">
								...
								<tr>
									<td>
										<select name="akun[]" id="akun" class="form-control akun" >
											...
											<?php
											$db = $this->db->query("select * from sr_account where modul_keuangan ='biaya' order by keterangan")->result();
											foreach ($db as $item):
												?>
												<option value="<?= $item->id_akun ?>"><?= $item->kode_akun . '-' . $item->keterangan ?></option>
											<?php endforeach; ?>
