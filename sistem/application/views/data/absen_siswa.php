								 ...
								 <div class="table-responsive">
                                    <table class="table bs-table table-striped table-bordered text-nowrap">
                                        <thead style="background-color:#042954;">
                                            <tr>
                                                <th style="color: #fff;" class="text-left">Siswa</th>
												<?php
											...
											<tr>
												...
												<td>
													<?php
														...
														$rowtanggal  = $this->db->query("SELECT A.tanggal_absen, A.idusers, A.tahun_ajaran, DATE_FORMAT(A.tanggal_absen,'%d') as shortdate FROM sr_absen A WHERE A.idusers='$idnya' AND type='SISWA' AND A.tanggal_absen = '".$looping."'")->result_array();
                                            ...
                                            <!--<tr>
                                                ...
                                                <td>
													<?php
														...
														$looping		 = $tgl_skr->format("Y-m-d");
														//$rowtanggal  = $this->db->query("SELECT * FROM sr_absen WHERE idusers='$idnya' AND DATE_FORMAT(tanggal_absen,'%d') = '".$i."'")->result_array();
														$rowtanggal  = $this->db->query("SELECT B.idkelas, A.tanggal_absen, A.idusers, A.tahun_ajaran, DATE_FORMAT(A.tanggal_absen,'%d') as shortdate FROM sr_absen A
														LEFT JOIN sr_siswa B on B.idsiswa = A.idusers
														LEFT JOIN sr_kelas C on C.idkelas = B.idkelas
														WHERE B.idkelas='$walikelas' AND A.idusers='$idnya' AND A.tanggal_absen = '".$looping."'")->result_array();
														foreach ($rowtanggal as $row1) {
													?>
															<i class="fas fa-check text-success"></i>
													<?php } ?>
