...
class Das extends CI_Controller
{
	...
	{
		parent::__construct();
//        if ($this->session->userdata('level') != "admin_group"){
//            redirect(base_url());
//        } else {
...
//		$d['combo_user'] = $this->Combo_model->combo_user();
//		$d['combo_tahun_ajaran'] = $this->Combo_model->combo_tahun_ajaran_only($tahun_ajaran);
		$d['das_kategori'] = $this->db->query("SELECT * FROM das_kategori ORDER BY id_das_kategori DESC");
		$d['page'] = 'das_kategori';
		$this->load->view('template', $d);
	}
...
//		$in['tahun_ajaran'] = $this->input->post("tahun_ajaran");
	...
	public function das_kategori_hapus($id_das_kategori)
	{
		...
		$cek = $this->db->query("SELECT * FROM das_kategori WHERE id_das_kategori = '$id_das_kategori'");
//		if ($cek->num_rows() > 0) {
